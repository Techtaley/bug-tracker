...
export default function Home() {  //need to define action HERE not in handle before using it
  ...
  const { items } = useSelector(state => state.bugs)  //from reducer's store global state in index.js - gets all bugs
...
//1. displays a list of bugs on the FE when page logs 
useEffect(() => {  
  dispatch(getBugsAsync(items))  //can move to slice and create a getdata() function and dispatch()
}, [items, dispatch]) //re-renders if dispatch updates

//2. adds new bugs
useEffect(() => {
  dispatch(sendBugsAsync())
